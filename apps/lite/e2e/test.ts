...
import { createLiteTestEnvironment, paths, processEnvironment, seedScenario } from "./setup.ts";
...
export const test = base.extend<TestOptions & TestFixtures>({
	...
	testEnvironment: async ({ scenario }, provide, testInfo) => {
		const environment = createLiteTestEnvironment(testInfo);
